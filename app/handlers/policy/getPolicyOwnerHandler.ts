...
export function getPolicyOwner(policyId: string): User {
...
export function findPolicyById(source: PolicySrc[], id: string): Policy {
...
export function findUserById(source: UserSrc[], id: string): User {
    for (let i = 0; i < source.length; i++) {
        if (source[i].id === id) {
